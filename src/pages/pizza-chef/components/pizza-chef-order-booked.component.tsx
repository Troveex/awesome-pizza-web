...
const PizzaChefOrderBooked: React.FC<IProps> = (props: IProps) => {
    ...
    const { startOrder, deleteOrder, getSearchOrderResponse, orderBooked, orderInProgress } = props;
        ...
        <Grid marginY={2}>
            ...
            {orderBooked ?
                getSearchOrderResponse?.response?.filter((item => !item.deleted && item.status === StatusOrderEnum.BOOKED)).map((item: OrderDto, idx: number) => (
                    <PizzaChefOrderComponent key={idx} order={item} startOrder={startOrder} deleteOrder={deleteOrder} orderInProgress={orderInProgress} />
                )) :
                <PizzaChefOrderEmpty />
            }
        </Grid>
